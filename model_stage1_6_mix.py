...
class WavletMono_unit_add_lvl1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, range_flow=1, is_train=True, imgshape1=(20, 23, 18)):
    ...
    def forward(self, x1, y1):
        ...
        fea_block5 = self.resblock5(fea_down4)
        fea_up1 = self.up1(fea_block5)
        fea_block6 = self.resblock5(fea_up1)
        fea_up2 = self.up2(fea_block6)
        fea_up2 = fea_up2[:,:,:,0:23,1:19]
        skip1 = self.skip(fea_block3)
        skip2 = self.skip(skip1)
        field1_v=self.output_lvl1(torch.cat([skip2,fea_up2],dim=1)) * self.range_flow
        field1 = self.diff_transform(field1_v, self.grid_1, self.range_flow) #deformation field
        field1_inv = self.diff_transform(-field1_v, self.grid_1, self.range_flow) #inverse deformation field
        warped_x1 = self.transform(x1, field1.permute(0, 2, 3, 4, 1), self.grid_1)
        warped_y1 = self.transform(x1, field1_inv.permute(0, 2, 3, 4, 1), self.grid_1)

        if self.is_train is True:
            return field1,  field1_inv, field1_v, warped_x1,  warped_y1
        else:
            return field1,  field1_inv, field1_v, warped_x1,  warped_y1
...
class WavletMono_unit_add_lvl2(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, range_flow=1, is_train=True, imgshape1=(20, 23, 18),imgshape2=(40, 46, 36),model1=None):
    ...
    def forward(self, x1, y1, x2_lll, y2_lll, x2_hhh, y2_hhh):
        #level 1
        field1,  field1_inv, field1_v, warped_x1,  warped_y1 = self.model1(x1, y1)

        #Level 2
        field1_v_up = self.up_tri(field1_v)
        warped_x2_lll_input = self.transform(x2_lll,  field1_v_up.permute(0, 2, 3, 4, 1), self.grid_2)

        cat_input_lvl2_lll = torch.cat((warped_x2_lll_input, y2_lll), 1)
        fixed2_lll = cat_input_lvl2_lll[:, 1:2, :, :, :]  # template
        mov2 = x2_lll  # moving
        fea_input2_lll = self.imgInput2(cat_input_lvl2_lll)

        warped_x2_hhh_input = self.transform(x2_hhh,  field1_v_up.permute(0, 2, 3, 4, 1), self.grid_2)
        ...
        fea_block52 = self.resblock5(fea_down42)
        fea_up12 = self.up1(fea_block52)
        fea_block62 = self.resblock6(fea_up12)
        fea_up22 = self.up2(fea_block62)
        fea_up22 = fea_up22[:,:,:,0:23,1:19]
        fea_block7 = self.resblock7(fea_up22)
        fea_up3 = self.up3(fea_block7)
        skip1 = self.skip(fea_block2)
        skip2 = self.skip(skip1)
        field2_v = self.output_lvl1(torch.cat([skip2, fea_up3], dim=1)) * self.range_flow
        ##addition
        #field2_v = field2_v + field1_v_up

        ##composition
        field1v_up_warp1 = self.transform( field1_v_up[:,0:1,:,:,:], field2_v.permute(0, 2, 3, 4, 1), self.grid_2)
        field1v_up_warp2 = self.transform( field1_v_up[:, 1:2, :, :, :], field2_v.permute(0, 2, 3, 4, 1), self.grid_2)
        field1v_up_warp3 = self.transform( field1_v_up[:, 2:3, :, :, :], field2_v.permute(0, 2, 3, 4, 1), self.grid_2)
        field1v_up_warp = torch.cat((field1v_up_warp1, field1v_up_warp2,field1v_up_warp3), 1)
        field2_v = field2_v + field1v_up_warp
        field2 = self.diff_transform(field2_v, self.grid_2, self.range_flow) #deformation field
        field2_inv = self.diff_transform(-field2_v, self.grid_2, self.range_flow) #inverse deformation field
        ...
        warped_y2_lll = self.transform(y2_lll, field2_inv.permute(0, 2, 3, 4, 1), self.grid_2)
        warped_y2_hhh = self.transform(y2_hhh, field2_inv.permute(0, 2, 3, 4, 1), self.grid_2)

        if self.is_train is True:
            return field1_v, field2_v, field2,  field2_inv, warped_x2_lll, warped_x2_hhh,  warped_y2_lll, warped_y2_hhh
        else:
            return field2

class WavletMono_unit_add_lvl3(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, range_flow=1, is_train=True, imgshape1=(20, 23, 18),imgshape2=(40, 46, 36),imgshape3=(80, 92, 72), model2 = None):
        ...
        self.imgInput3_2 = nn.Conv3d(self.in_channel, self.start_channel * 4, 3, stride=1, padding=1, bias=bias_opt)
    ...
    def forward(self, x1, y1, x2_lll, y2_lll, x2_hhh, y2_hhh,  x3_lll, y3_lll, x3_hhh, y3_hhh):
        #level 1
        #Level 2
        field1_v, field2_v, field2,  field2_inv, warped_x2_lll, warped_x2_hhh,  warped_y2_lll, warped_y2_hhh \
            = self.model2(x1, y1, x2_lll, y2_lll, x2_hhh, y2_hhh)

        # Level 3
        field2v_up = self.up_tri(field2_v)
        warped_x3_lll_input = self.transform(x3_lll, field2v_up.permute(0, 2, 3, 4, 1), self.grid_3)
        ...
        # diff_norm = torch.squeeze(diff_norm)
        # diff_norm = torch.squeeze(diff_norm)
        diff_Mhigh = ((1 - 0.5) * diff_norm + 0.5) * warped_x3_lll_input
        diff_Fhigh = ((1 - 0.5) * diff_norm + 0.5) * y3_lll

        cat_input_lvl3_lll = torch.cat((diff_Mhigh, diff_Fhigh), 1)
        fixed3_lll = y3_lll  # template
        mov3 = x3_lll  # moving
        fea_input3_lllx = self.imgInput3_2(cat_input_lvl3_lll)
        ...
        cat_input_lvl3_hhh = torch.cat((warped_x3_hhh_input, y3_hhh), 1)
        ...
        field3_v = self.output_lvl1(torch.cat([skip3, fea_up4], dim=1)) * self.range_flow
        ##addition
        #field3 = field3_v + field2v_up

        ##composition
        field2v_up_warp1 = self.transform(field2v_up[:, 0:1, :, :, :], field3_v.permute(0, 2, 3, 4, 1), self.grid_3)
        field2v_up_warp2 = self.transform(field2v_up[:, 1:2, :, :, :], field3_v.permute(0, 2, 3, 4, 1), self.grid_3)
        field2v_up_warp3 = self.transform(field2v_up[:, 2:3, :, :, :], field3_v.permute(0, 2, 3, 4, 1), self.grid_3)
        field2v_up_warp = torch.cat((field2v_up_warp1, field2v_up_warp2, field2v_up_warp3), 1)
        field3_v = field3_v + field2v_up_warp

        field3 = self.diff_transform(field3_v, self.grid_3, self.range_flow) #deformation field
        field3_inv = self.diff_transform(-field3_v, self.grid_3, self.range_flow) #inverse deformation field
        ...
        warped_y3_lll = self.transform(y3_lll, field3_inv.permute(0, 2, 3, 4, 1), self.grid_3)
        warped_y3_hhh = self.transform(y3_hhh, field3_inv.permute(0, 2, 3, 4, 1), self.grid_3)


        if self.is_train is True:
            return field1_v, field2_v, field3_v,  field3, field3_inv, warped_x3_lll, warped_x3_hhh,  warped_y3_lll, warped_y3_hhh
        else:
            return field3

class WavletMono_unit_add_lvl4(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, range_flow = 1, is_train=True, imgshape1=(20, 23, 18),imgshape2=(40, 46, 36),imgshape3=(80, 92, 72),imgshape4=(160, 184, 144), model3=None):
        ...
        self.imgInput1 = nn.Conv3d(self.in_channel, self.start_channel * 4, 3, stride=1, padding=1, bias=bias_opt)
        self.imgInput2 = nn.Conv3d(self.in_channel, self.start_channel * 2, 3, stride=1, padding=1, bias=bias_opt)
        self.imgInput3 = nn.Conv3d(self.in_channel, self.start_channel * 2, 3, stride=1, padding=1, bias=bias_opt)
        self.imgInput4_2 = nn.Conv3d(self.in_channel, self.start_channel * 4, 3, stride=1, padding=1, bias=bias_opt)
    ...
    # def forward(self, cat12):
    #     x1 = cat12[:, 0:1, 0:20, 0:23, 0:18]
    #     y1 = cat12[:, 1:2, 0:20, 0:23, 0:18]
    #     x2_lll = cat12[:, 2:3, 0:40, 0:46, 0:36]
    #     y2_lll = cat12[:, 3:4, 0:40, 0:46, 0:36]
    #     x2_hhh = cat12[:, 4:5, 0:40, 0:46, 0:36]
    #     y2_hhh = cat12[:, 5:6, 0:40, 0:46, 0:36]
    #     x3_lll = cat12[:, 6:7, 0:80, 0:92, 0:72]
    #     y3_lll = cat12[:, 7:8, 0:80, 0:92, 0:72]
    #     x3_hhh = cat12[:, 8:9, 0:80, 0:92, 0:72]
    #     y3_hhh = cat12[:, 9:10, 0:80, 0:92, 0:72]
    #     source1 = cat12[:, 10:11, :, :, :]
    #     source2 = cat12[:, 11:12, :, :, :]
        #level 1
        #Level 2
        # Level 3
        field1_v, field2_v,  field3_v, field3, field3_inv, warped_x3_lll, warped_x3_hhh,  warped_y3_lll, warped_y3_hhh\
            = self.model3(x1, y1, x2_lll, y2_lll, x2_hhh, y2_hhh,  x3_lll, y3_lll, x3_hhh, y3_hhh)
        x = self.model3
        field1v_up = self.up_tri(field1_v)
        field2v_up = self.up_tri(field2_v)
        # Level 4(source)
        field3v_up = self.up_tri(field3_v)
        warped_source1_input = self.transform(source1, field3v_up.permute(0, 2, 3, 4, 1), self.grid_4)
        ...
        cat_input_lvl4_source = torch.cat((diff_Mhigh,  diff_Fhigh), 1)
        fixed4_source2 = source2  # template
        fea_input4_source = self.imgInput4_2(cat_input_lvl4_source)
        ...
        field4_v = self.output_lvl1(torch.cat([skip1, fea_up5], dim=1)) * self.range_flow
        ## addition
        #field4 = field4 + field3_up

        ##composition
        field3v_up_warp1 = self.transform(field3v_up[:, 0:1, :, :, :], field4_v.permute(0, 2, 3, 4, 1), self.grid_4)
        field3v_up_warp2 = self.transform(field3v_up[:, 1:2, :, :, :], field4_v.permute(0, 2, 3, 4, 1), self.grid_4)
        field3v_up_warp3 = self.transform(field3v_up[:, 2:3, :, :, :], field4_v.permute(0, 2, 3, 4, 1), self.grid_4)
        field3v_up_warp = torch.cat((field3v_up_warp1, field3v_up_warp2, field3v_up_warp3), 1)
        field4_v = field4_v + field3v_up_warp
        field4 = self.diff_transform(field4_v, self.grid_4, self.range_flow) #deformation field
        field4_inv = self.diff_transform(-field4_v, self.grid_4, self.range_flow) #inverse deformation field

        warped_source_x = self.transform(source1, field4.permute(0, 2, 3, 4, 1), self.grid_4)
        warped_source_y = self.transform(source1, field4_inv.permute(0, 2, 3, 4, 1), self.grid_4)


        if self.is_train is True:
            return field1_v, field2_v, field3_v, field4_v,  field4, field4_inv, warped_source_x, warped_source_y
        else:
            return field4, warped_source_x, warped_source_y
